...
float4 NLQu( float3 uvw, Texture3D noiseTex ) {
  return noiseTex.SampleLevel(_LinearRepeatSampler, uvw, 0);
}
...
float NHQu(float3 uvw, Texture3D tex, float smooth = 1) 
{
  ...
  float2 d = float2( INV_LATTICE_SIZE, 0 );
    ...
    float4 f1 = tex.SampleLevel(_LinearRepeatSampler, uvw2        , 0).zxyw; // <+0, +y, +z, +yz>
    float4 f2 = tex.SampleLevel(_LinearRepeatSampler, uvw2 + d.xyy, 0).zxyw; // <+x, +xy, +xz, +xyz>
